...
LANG, TRADE_TYPE, AMOUNT, PAYMENT, SCREENSHOT, WALLET_ADDRESS, SELL_SCREENSHOT, SELL_TELEBIRR, SUPPORT_MSG = range(9)
...
# Permanent Bottom Reply Keyboard
...
TEXTS = {
    'am': {
        ...
        'got_ss_sell': (
            ...
            "───────────────────\n"
            "📞 ብር ገቢ የሚደረግበትን የ <b>Telebirr የስልክ ቁጥር</b> እና <b>ስም</b> ይፃፉልን፦"
        ),
        'complete': (
            "🎉 <b>ትዕዛዝዎ በስኬት ተላኳል!</b>\n"
            "───────────────────\n"
    ...
    'en': {
        ...
        'got_ss_buy': "📥 <b>Payment receipt received!</b> ✅\n\nEnter your <b>Wallet Address</b> to receive USD:",
        'got_ss_sell': "📥 <b>Transfer proof received!</b> ✅\n\nEnter your <b>Telebirr Phone Number & Name</b> to receive payment:",
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if update.message:
        ...
        await update.message.reply_text("💡 <i>ማሳሰቢያ፦ ከስር ያለውን ቋሚ ቁልፍ በመጫን በማንኛውም ሰዓት ወደ መጀመሪያው መመለስ ይችላሉ።</i>", reply_markup=PERMANENT_KEYBOARD, parse_mode="HTML")
    return LANG
...
async def select_trade_type(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if trade_type == 'buy':
        msg = t['enter_amount_buy'].format(min=MIN_USD, max=MAX_USD)
    else:
        msg = t['enter_amount_sell'].format(min=MIN_USD, max=MAX_USD)
...
async def get_amount(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if trade_type == 'buy':
            msg = t['summary_buy'].format(usd=usd_amount, birr=total_birr)
        else:
            msg = t['summary_sell'].format(usd=usd_amount, birr=total_birr)
...
async def receive_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if user and photo_file:
        ...
        admin_msg = (
            ...
            f"📍 <b>ተጠቃሚው የሰጠው Wallet:</b> <code>{wallet_address}</code>\n"
            f"───────────────────"
        )
        admin_keyboard = [
...
async def receive_sell_telebirr(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if user and photo_file:
        ...
        admin_msg = (
            ...
            f"📱 <b>የተጠቃሚው Telebirr Info:</b> <code>{telebirr_info}</code>\n"
            f"───────────────────"
        )
        admin_keyboard = [
...
def main() -> None:
    ...
    print("Bot is running with updated TRC20 wallet address...")
    app.run_polling()
